...
export default function transform (options: Options): TransformResult {
  ...
  const ast = parse(code, {
    ...
    ].concat(process.env.ESLINT === 'false' || options.isNormal || options.isTyped || process.env.NODE_ENV === 'test' ? [] : eslintValidation).concat((process.env.NODE_ENV === 'test') ? [] : require('babel-plugin-remove-dead-code').default)
  }).ast as t.File
  if (options.isNormal) {
    return { ast } as any
  }
