...
import { convertSourceStringToAstExpression as toAst, getObjKey } from './util/astConvert'
import {
  PROJECT_CONFIG,
  REG_TYPESCRIPT,
  REG_SCRIPTS,
...
function processEntry (code, filePath) {
    ...
    ObjectProperty (astPath) {
      ...
      if (keyName === 'pages' && t.isArrayExpression(value)) {
        ...
        value.properties.forEach(node => {
          if ((node as any).keyName === 'position') tabbarPos = (node as any).value.value
        })
  ...
  traverse(ast, {
    ...
    ClassProperty: {
      enter (astPath) {
        const node = astPath.node as t.ClassProperty
        const key = node.key
        const value = node.value
        const keyName = getObjKey(key)
      ...
      enter (astPath) {
        ...
        if (!Util.isNpmPkg(value)) {
          if (value.indexOf('.') === 0) {
            ...
            if (pathArr.indexOf('pages') >= 0) {
              astPath.remove()
            } else if (REG_SCRIPTS.test(value)) {
        ...
        if (value === PACKAGES['@tarojs/taro']) {
          ...
          specifiers.forEach((item, index) => {
            if ((item as any).imported && taroApis.indexOf((item as any).imported.name) >= 0) {
              taroApisSpecifiers.push(t.importSpecifier(t.identifier(item.local.name), t.identifier((item as any).imported.name)))
    ...
    CallExpression: {
      enter (astPath) {
        const node = astPath.node as t.CallExpression
    ...
    Program: {
      exit (astPath) {
        ...
        const lastImportIndex = _.findLastIndex(node.body, t.isImportDeclaration)
        const lastImportNode = astPath.get(`body.${lastImportIndex > -1 ? lastImportIndex : 0}`) as NodePath
        const extraNodes = [
          importTaroH5Node,
          importRouterNode,
          initPxTransformNode
        ...
        if (renderCallCode) {
          ...
          node.body.push(renderCallNode as any)
        }
      }
    }
  })
        ...
        if (!Util.isNpmPkg(value)) {
          ...
          specifiers.forEach((item, index) => {
            if ((item as t.ImportSpecifier).imported && taroApis.indexOf((item as t.ImportSpecifier).imported.name) >= 0) {
              taroApisSpecifiers.push(t.importSpecifier(t.identifier(item.local.name), t.identifier((item as t.ImportSpecifier).imported.name)))
              deletedIdx.push(index)
            }
          })
          _.pullAt(specifiers, deletedIdx)
...
function classifyFiles (filename) {
  ...
  const isPage: boolean = pages.some(page => {
    ...
    klaw(sourcePath)
      .on('data', file => {
        ...
        if (!file.stats.isDirectory()) {
          Util.printLog(processTypeEnum.CREATE, '发现文件', relativePath)
